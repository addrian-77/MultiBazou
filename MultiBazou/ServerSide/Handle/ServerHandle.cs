...
namespace MultiBazou.ServerSide.Handle
{
    public class ServerHandle
    {
        ...
        public static void ItemPickup(int fromClient, Packet packet)
        {
            int itemId = packet.ReadInt();
            ItemManager.PickupItem(itemId, fromClient);
        }

        public static void ItemDrop(int fromClient, Packet packet)
        {
            int itemId = packet.ReadInt();
            Vector3Serializable pos = packet.Read<Vector3Serializable>();
            ItemManager.DropItem(itemId, pos.ToVector3());
        }    
